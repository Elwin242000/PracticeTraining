 ...
 * A Value Object that is created in the related event and performs the role of data delivery<br> 
 * when a server execution request is made.
 *
 * @author 
 * @since J2EE 1.6
...
public class SummaryVO extends AbstractValueObject {
	...
	/* Column Info */
	private String acctYrmonFm = null;
	/* Column Info */
	 ...
	 * @param acctYrmonFm
	 */
	public void setAcctYrmonFr(String acctYrmonFm) {
		this.acctYrmonFm = acctYrmonFm;
	}
	...
	/**
	 * Extract the data of the request and set it in the member variable of the VO.
	 * @param request
	 */
	public void fromRequest(HttpServletRequest request, String prefix) {
		...
		setAproFlg(JSPUtil.getParameter(request, prefix + "acct_yrmon_fr", ""));
		setAproFlg(JSPUtil.getParameter(request, prefix + "acct_yrmon_to", ""));
		setAproFlg(JSPUtil.getParameter(request, prefix + "trd_cd", ""));
	}
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				if (acctYrmonFr[i] != null)
					model.setAproFlg(acctYrmonFr[i]);
				if (acctYrmonTo[i] != null)
					model.setAproFlg(acctYrmonTo[i]);
				if (trdCd[i] != null)
					model.setAproFlg(trdCd[i]);
				models.add(model);
			}
	...
	*/
	public void unDataFormat(){
		...
		this.acctYrmonFm = this.acctYrmonFm .replaceAll(",", "").replaceAll("-", "").replaceAll("/", "").replaceAll(":", "");
